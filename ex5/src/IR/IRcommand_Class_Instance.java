...
public class IRcommand_Class_Instance extends IRcommand
{
    ...
    public void MIPSme()
    {
        ...
        //  allocate for size of dataMembers + 1 for Dispatch Vectors
        ...
        //now fill default values for each data_member
        for(TYPE_CLASS_VAR_DEC_LIST curr_node = data_members, curr_node != null, curr_node=curr_node.tail)
        {
            ...
            if(curr_var.TYPE instanceof TYPE_STRING)
            {
            ...
            {
                MIPSGenerator.getInstance().la(s0, curr_var.defaultIntValue);
                MIPSGenerator.getInstance().sw(s0, 4*(i+4), this.pointer);
            }
        }
